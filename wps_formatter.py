...
class WRFInputFormatter:
    ...
    def add_to_WPS(self, field, soil_levels=None):
        ...
        for tnum, t in enumerate(list(field["time"])):
            ...
            elif (field.name in ["tsoil", "soilw", "mrlsl", "soilt", "tsl", 'soill']) or (
    ...
    def get_pressure_from_pressure(self):
        ...
        ta_subset = self.load_and_subset(var_name="air")
    ...
    def process_soil_levels(self, varray, ismoist=False, interpolate_to=None):
        """
...
if __name__ == "__main__":
    ...
    # file_dir = '/Users/lmadaus/esg-cmip-downloads/HadGEM2-ES'
    ...
    # Options are 'pressure', 'hybrid-p','hybrid-z'
    vcoord_type = "pressure"
    # Do we need to convert from hybrid height coordinates?
    # So far HadGEM2-ES is the only one that needs this
    # convert_from_hybrid = False
    # List of pressure levels to interpolate to
    # Here, just 30 arbitrary levels between 100 and 1000 mb
    # plevs = list(np.linspace(10000.0, 100000., 30))

    # Build the object
    wrfgen = WRFInputFormatter(
        variable_table=grib_codes,
        master_path=file_dir,
        time_range=time_range,
        time_freq=time_freq,
        vcoord_type=vcoord_type,
    )

    # Set up the WPS output files
    wrfgen.open_WPS_files()

    # Process air temperature
    # Load in the ta field and reduce it to just the times
    # we need for this run
    ta = wrfgen.load_and_subset("ta")
    # Interpolate to pressure levels
    ta = wrfgen.interp_to_p_levels(ta)
    # To inspect the size of the file after interpolation/resampling
    # ta.to_netcdf('ta_subset.nc')
    wrfgen.add_to_WPS(ta)

    # Process sea-level pressure
    # For single levels like this, no need
    # to interpolate to pressure levels
    psl = wrfgen.load_and_subset("psl")
    # psl.to_netcdf('psl_subset.nc')
    wrfgen.add_to_WPS(psl)
    """
    Total variables to process
    ta -> air temperature (3d, 6-hourly)
    hus -> Specific humidity (3d, 6-hourly)
    ua -> u-wind component (3d, 6-hourly)
    va -> v-wind component (3d, 6-hourly)
    psl -> sea-level pressure (single level, 6-hourly)
    ps -> surface pressure (single level, 6-hourly)
    tsl -> Soil temperature (Monthly, 4 levels)
    mrlsl -> Soil moisture (Monthly, 4 levels)
    tsk -> Skin (sea-surface) temperature (daily, single level)
